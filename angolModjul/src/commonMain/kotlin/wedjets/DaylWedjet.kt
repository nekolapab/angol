...
fun DaylWedjet(
    ...
    val filteredModyilz = modjilz.filter { it.parentId == activeParentId || it.id == if (activeParentId == "dayl") "angol" else "dayl" }
    ...
    val gredItems = filteredModyilz.mapNotNull { mod ->
        ...
        GredUydem(
            ...
            label = label,
            color = finalColor,
            isFolder = (mod.type == "kepad" || mod.type == "RebeldModjul" || mod.type == "beld" || mod.id == "beldir"),
            deyda = mod
        )
    }.toMutableList()
